...
public class Main implements Runnable {
	...
	public void run() {
		...
		int rows = cellsSize[0];
		int cols = cellsSize[1];
		// create a window
	...
	private int getRowsOrCols(String target) {
		...
		while (isInputValid == false) {
			String inputTarget = JOptionPane.showInputDialog(
					"Please input " + target + " of cells (values must be integer 1 to 100, default is 20)");
			if (inputTarget == null) {
				return defaultnum;
			}
			try {
